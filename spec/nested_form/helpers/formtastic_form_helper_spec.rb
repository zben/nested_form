...
describe NestedForm::Helpers::FormtasticFormHelper do
  ...
  it "should pass nested form builder to form_for along with other options" do
    pending
    mock.proxy(_view).form_for(:first, :as => :second, :other => :arg, :builder => NestedForm::Builders::FormtasticFormBuilder) do |form_html|
      form_html
    end
  ...
  it "should pass instance of NestedForm::Builder to nested_form_for block" do
    _view.semantic_nested_form_for(Project.new) do |f|
      f.should be_instance_of(NestedForm::Builders::FormtasticFormBuilder)
    end
  end
